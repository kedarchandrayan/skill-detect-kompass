const rootPrefix = '..',
  ...
  rollbarNotifier = require(rootPrefix + '/lib/rollbarNotifier'),
...
class CustomError {
  ...
  /**
   * Send an alert to rollbar.
   * @returns {Promise<void>}
   */
  async notify() {
    const oThis = this;

    const rollbarMessageUuid = await rollbarNotifier.error(oThis.message, {
      errorId: oThis.errorId,
      debugOptions: oThis.debugOptions,
      message: oThis.message,
      code: oThis.code
    });

    logger.log('Rollbar response: ', rollbarMessageUuid, oThis.message);
  }
