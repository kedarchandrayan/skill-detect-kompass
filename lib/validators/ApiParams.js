const rootPrefix = '../..',
  ...
  missionApiSignature = require(rootPrefix + '/config/apiParams/missions/signature'),
...
class ApiParamsValidator {
  ...
  async _setParamsSignature() {
    ...
    apisSignature = missionApiSignature;

    oThis.paramSignature = apisSignature[oThis.apiName];

    if (!oThis.paramSignature) {
      return Promise.reject(standardResponse.error('l_v_ap_fpc_2', { apiName: oThis.apiName }));
    }
  }
  ...
  async _validateMandatoryParams() {
    ...
    for (const paramConfig of mandatoryParamConfigs) {
      ...
      let paramSource =
        paramConfig.kind && paramConfig.kind === 'internal' ? oThis.internalParams : oThis.externalParams;

      // Performing the mandatory check
      const isParamPresentAndHasValue =
        Object.prototype.hasOwnProperty.call(paramSource, paramName) &&
