...
class TaskSplitter {
  ...
  async _populateMissionDetails() {
    ...
    for (const ele of totalWorkExObj) {
      strParts.push(`${he.decode(ele.op)} ${ele.val}`);
    }
    ...
    const skillsRow = ['Skills', skillsArr.join(', ')];
    ...
    for (const skill of skillsArr) {
      resumeInfoHeaderRow.push(skill);
    }

    resumeInfoHeaderRow.push('Custom Requirement Observations');
