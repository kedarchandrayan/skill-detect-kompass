...
class Rover {
  ...
  async _appendToReport() {
    ...
    for(const criterion of criteria) {
      const criterionParsedData = {
        criterion: criterion.criterion_name,
        highlight: criterion.criterion_summary
      };

      if(criterion.is_matched) {
        positives.push(criterionParsedData);
      } else {
        negatives.push(criterionParsedData);
      }
    }
    ...
    await new SpreadSheetHelper().appendRows(oThis.reportSsId, sheetName, [
      [
        name, email, phoneNumber, totalWorkEx, confidence,
        JSON.stringify(positives, null, 4), JSON.stringify(negatives, null, 4)
      ]
    ]);
  }
  ...
  _parseTotalExperience() {
    ...
    for (const totalExElement of totalExperienceArray) {
      parsedArray.push(`${totalExElement.op} ${totalExElement.val}`);
    }
