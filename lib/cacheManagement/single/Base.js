...
/**
 * Base class to manage single cache.
 * All single cache classes to inherit from this class.
 *
 * @class GetCacheSingleBase
 */
class GetCacheSingleBase {
  /**
   * Constructor
  ...
  async _setPaginationCacheKey() {
    ...
    if (!versionDetail || !versionCacheResponse.isSuccess()) {
      // Set version cache.
      versionDetail = {
        v: uuidV4()
      };
  ...
  async _setCache(dataToSet) {
    ...
    return setCacheFunction().then(async function (cacheSetResponse) {
      if (!cacheSetResponse.isSuccess()) {
        return standardResponse.error('cache_not_set:l_cm_s_b_sc_1', { key: oThis.cacheKey, dataToSet: dataToSet });
      }
    });
  }
...
module.exports = GetCacheSingleBase;
